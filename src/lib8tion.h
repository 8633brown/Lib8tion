...
LIB8STATIC float sfract15ToFloat(sfract15 y) { return y / 32768.0; }
...
LIB8STATIC sfract15 floatToSfract15(float f) { return f * 32768.0; }
...
LIB8STATIC uint8_t ease8InOutQuad(uint8_t val) {
  ...
  asm volatile(
      ...
      "rol %[j]       \n" // j = (j * 2) + carry // preserve add'l bit of
                          // precision
      "sbrc %[val], 7 \n"
      "com %[j]       \n"
      "clr __zero_reg__   \n"
...
static uint32_t operator*(uint32_t v, q<T, F, I> &q) {
  return q * v;
}
template <class T, int F, int I>
static uint16_t operator*(uint16_t v, q<T, F, I> &q) {
  return q * v;
}
template <class T, int F, int I>
static int32_t operator*(int32_t v, q<T, F, I> &q) {
  return q * v;
}
template <class T, int F, int I>
static int16_t operator*(int16_t v, q<T, F, I> &q) {
  return q * v;
}
#ifdef FASTLED_ARM
...
//
//  Originally designed to make an entire animation project pulse with
//  brightness. For that effect, add this line just above your existing call to
//  "FastLED.show()":
//
...
// by #defining USE_GET_MILLISECOND_TIMER.
    ...
    !defined(USE_GET_MILLISECOND_TIMER)
...
LIB8STATIC uint16_t beatsin88(accum88 beats_per_minute_88, uint16_t lowest = 0,
...
///           that oscillates within a given range.
LIB8STATIC uint16_t beatsin16(accum88 beats_per_minute, uint16_t lowest = 0,
                              uint16_t highest = 65535, uint32_t timebase = 0,
                              uint16_t phase_offset = 0) {
...
///           that oscillates within a given range.
LIB8STATIC uint8_t beatsin8(accum88 beats_per_minute, uint8_t lowest = 0,
...
#define EVERY_N_BSECONDS(N)                                                    \
  EVERY_N_BSECONDS_I(CONCAT_MACRO(PER, __COUNTER__), N)
